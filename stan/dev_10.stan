...
transformed parameters {
  ...
  vector[N] beta_a;
  for (n in 1:N) {
    beta_h[n] = alpha + eta_o[tid[n,1]] * sigma_o - eta_d[tid[n,2]] * sigma_d + eta_g[tid[n,1]] * sigma_g;
...
model {
  ...
  target += normal_lpdf(sigma_o | sigma_o_mu, sigma_o_sigma) - normal_lccdf(0 | sigma_o_mu, sigma_o_sigma);
  target += normal_lpdf(sigma_d | sigma_d_mu, sigma_d_sigma) - normal_lccdf(0 | sigma_d_mu, sigma_d_sigma);
  target += normal_lpdf(sigma_g | sigma_g_mu, sigma_g_sigma) - normal_lccdf(0 | sigma_g_mu, sigma_g_sigma);
