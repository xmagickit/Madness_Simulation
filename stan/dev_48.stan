functions {
  vector vectorize_parameters(real log_sigma_h,
                              real log_sigma,
                              vector beta0_h,
                              vector beta0,
                              matrix eta_h,
    ...
    params[1] = log_sigma_h;
    params[2] = log_sigma;
    params[3:(T+2)] = beta0_h;
    params[(T+3):(T+2+T)] = beta0;
  ...
  array[] vector map_lambda(matrix beta,
    ...
    for (t in 1:2) {
      for (n in 1:N) {
        log_lambda[t,n] = beta[tid[t,n], sid[n]] + beta_h[tid[t,n], sid[n]] * H[t,n];
      }
      log_lambda[t,:] += M;
    }
...
transformed parameters {
  ...
  vector[P] params = vectorize_parameters(log_sigma_h, log_sigma, beta0_h, beta0, eta_h, eta);
  array[2] vector[N] log_lambda = map_lambda(beta, beta_h, tid, sid, M, H);
}
